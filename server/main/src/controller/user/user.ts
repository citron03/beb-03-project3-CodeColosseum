...
const patch = async (req:any, res:any) => {
    ...
    try {
        ...
        if (body.nickName) {
            ...
            // 수정
            const updatedUser = await models.User.findOneAndUpdate({account}, {nickName:body.nickName}, {new:true});
            res.status(200).send({message: "nickName updated!", data:updatedUser});
        }
        else if (body.image) {
            res.status(400).send({message: "Sorry, Request to update image is not available yet!"});
        }
        else {
